...
# -*- coding:utf-8 -*-
#
#   The maximum SHAP score of the irrelevant feature and the minimum SHAP score of the relevant feature.
...
def plot_irr_rel_summary(data, len_X, fig_title, filename, score_type="lundberg"):
    ...
    plt.xlabel('SHAP scores' if score_type == "lundberg" else 'Shapley values')
    plt.title(fig_title)
    plt.savefig(filename)
    plt.clf()
    plt.cla()
    ...
    if len(args) >= 2 and args[0] == '-bench':
        ...
        for item in name_list:
            ...
            plot_max_irr_min_rel(np.array(max_irr_and_min_rel), column_names, name,
                                 f"shap_scores/all_points/IrrRel/{which_score}_scores/{name}.png",
                                 which_score)
            plot_irr_rel_summary(np.asarray([max_ir_ge_min_r, max_ir_lt_min_r]), ['Abnormal', 'Normal'], name,
                                 f"shap_scores/all_points/IrrRel/{which_score}_summary/{name}.png",
                                 which_score)
