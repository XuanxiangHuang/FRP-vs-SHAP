...
# -*- coding:utf-8 -*-
#
#   Compare the rankings of feature importance generated by Barcelo's algorithm and the SHAP tool.
...
if __name__ == '__main__':
    ...
    if len(args) >= 1 and args[0] == '-bench':
        ...
        for item in name_list:
            ...
            output.to_csv(f"shap_scores/all_points/wrong_pairs/b_lundberg/{name}.csv", index=False)
            plot_wrong_pairs(np.asarray(l_wrong_pairs), int((m * (m - 1)) / 2 + 1), name,
                             f"shap_scores/all_points/wrong_pairs/b_lundberg/{name}.png")
